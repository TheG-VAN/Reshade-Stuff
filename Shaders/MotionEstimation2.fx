...
{	
	...
	if (UI_DEBUG_ENABLE)
	{
		switch (UI_DEBUG_MODE)
		{
			...
			case 5:
			{
				returnValue =  motionToLgbtq(tex2D(SamplerMotionVectors2, texcoord).rg);
			}
			break;
			//Velocity Buffer
			case 6:
			{
				returnValue = float4(((tex2D(SamplerMotionVectors2, texcoord).rg * 0.5 * UI_DEBUG_MULT) + 0.5), 0, 0);
			}
			break;	
...
technique DRME2
{
	...
	pass MotionOutputPass
	{
		...
		RenderTarget = texMotionVectors2;
	}

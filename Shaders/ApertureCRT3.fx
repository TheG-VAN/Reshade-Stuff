...
uniform float LinesRatio <
	ui_type = "drag";
	ui_min = 0.0;
	ui_max = 1.0;
	ui_step = 0.05;
	ui_label = "Lines Ratio [CRT-Aperture]";
> = 0.5;
...
uniform float Lines <
	...
	ui_max = 1.0;
	ui_step = 0.05;
	ui_label = "Lines Strength [CRT-Aperture]";
> = 0.5;

uniform bool InvertLines <
	ui_type = "checkbox";
	ui_min = 0.0;
	ui_max = 1.0;
	ui_step = 1;
	ui_label = "Invert Lines [CRT-Aperture]";
> = 1;
...
uniform bool Lanczos <
	...
	ui_max = 1.0;
	ui_step = 1;
	ui_label = "Filter Lanczos [CRT-Aperture]";
> = 1;

#define FIX(c) max(abs(c), 1e-5)
#define PI 3.141592653589
#define TEX2D(c) pow(tex2D(tex, c).rgb, GAMMA_INPUT)
#define texture_size (tex2Dsize(ReShade::BackBuffer) / SIZE)
#define video_size (tex2Dsize(ReShade::BackBuffer) / SIZE)
...
float4 PS_CRTAperture(float4 vpos : SV_Position, float2 co : TEXCOORD0) : SV_Target
{
	...
	if (!Lanczos) {
		col = pow(tex2D(ReShade::BackBuffer, co).rgb, GAMMA_INPUT);
	}
	
	if (InvertLines) {
		float3 col1 = lerp(col, col * get_scanline_weight(frac(co.y * texture_size.y), col_soft), (2 - 2 * abs(LinesRatio - 0.5)) * LinesRatio * Lines);
		float3 col2 = lerp(col, col * get_scanline_weight(frac(co.x * texture_size.x), col_soft), (2 - 2 * abs(LinesRatio - 0.5)) * (1 - LinesRatio) * Lines);
		col = 0.5 * (col1 + col2);
	} else {
		col = lerp(col, col * get_scanline_weight(frac(co.y * texture_size.y), col_soft), (2 - 2 * abs(LinesRatio - 0.5)) * LinesRatio * Lines);
		col = lerp(col, col * get_scanline_weight(frac(co.x * texture_size.x), col_soft), (2 - 2 * abs(LinesRatio - 0.5)) * (1 - LinesRatio) * Lines);
    }
